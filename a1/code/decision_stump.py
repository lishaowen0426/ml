...
class DecisionStump:
    ...
    def fit(self, X,y):
        ...
        for d in range(D):
            for n in range(N):
                #read the feature value as a threshold value
                value = X[n,d]

                #find the most common label for satisfying and unsatisfying
        ...
        if np.unique(y).size > 1:
            ...
            for d in range(D):
                for n in range(N):
                    ...
                    y_sat = utils.mode(y[X[:,d] == value]) #the most common label when satisfied
                    y_not = utils.mode(y[X[:,d] != value])
                    # Make predictions
                    y_pred = y_sat * np.ones(N)
                    y_pred[X[:, d] != value] = y_not
