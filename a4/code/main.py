...
if __name__ == "__main__":
    ...
    if question == "2":
        ...
        # TODO
        L2_model = LogisticRegression(fit_intercept = False)
        L2_model.fit(XBin,yBin)
        print("\nL2 Training error %.3f" % utils.classification_error(L2_model.predict(XBin),yBin))
        print("L2 Validation error %.3f" % utils.classification_error(L2_model.predict(XBinValid), yBinValid))
        print("L2 # nonZeros: %d" % (L2_model.coef_ != 0).sum())

        L1_model = LogisticRegression(penalty ='l1' ,fit_intercept = False)
        L1_model.fit(XBin,yBin)
        print("\nL1 Training error %.3f" % utils.classification_error(L1_model.predict(XBin),yBin))
        print("L1 Validation error %.3f" % utils.classification_error(L1_model.predict(XBinValid), yBinValid))
        print("L1 # nonZeros: %d" % (L1_model.coef_ != 0).sum())
        ...
        # TODO
        #one vs all
        model = LogisticRegression(solver = 'lbfgs',C = 1e10 ,fit_intercept = False)
        model.fit(XMulti,yMulti)
        print("\none vs all Training error %.3f" % utils.classification_error(model.predict(XMulti),yMulti))
        print("Validation error %.3f" % utils.classification_error(model.predict(XMultiValid), yMultiValid))

         #softmax
        model = LogisticRegression(multi_class = 'multinomial',solver = 'lbfgs',C = 1e10 ,fit_intercept = False)
        model.fit(XMulti,yMulti)
        print("\nsoftmax Training error %.3f" % utils.classification_error(model.predict(XMulti),yMulti))
        print("Validation error %.3f" % utils.classification_error(model.predict(XMultiValid), yMultiValid))
